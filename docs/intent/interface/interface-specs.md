...
- [x] **UI-GRAPH3D-013**: The graph shall be drawn on a dark ground declared for it, and each skill shall carry its state's colour from the graph's declared palette rather than from the page palette.
- [x] **UI-GRAPH3D-017**: The graph's palette shall be declared once as the mapping from node states to graph colours, mirrored as tokens, and every actionable colour shall clear 3:1 against the graph's ground; locked and section colours shall be permitted to recede below that line because the state is carried by label, hover and the keyboard list.
...
- [x] **UI-GRAPH3D-025**: A realm's lesson chain shall be walked neighbour to neighbour: standing at a lesson, the lesson the line connects it to — the one ahead and the one behind — shall carry a floating card naming it, showing what it asks for and its current progress, and clicking an open card shall move the camera to that lesson, which becomes the new standing point; a closed card shall explain the prerequisite instead of moving the camera, and the test shall hang above the last lesson as its own card.
