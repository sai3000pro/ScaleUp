...
- [x] **UI-GRAPH3D-017**: The graph's palette shall be declared once as the mapping from node states to graph colours, mirrored as tokens, and every state colour shall clear 3:1 against the light graph ground; locked and section colours shall remain explicitly labelled and may recede visually because the state is carried by text and shape.
- [x] **UI-GRAPH3D-018**: The legend rendered beside the graph shall present the graph's palette, so the key describes the orbs the learner is looking at.
- [x] **UI-GRAPH3D-019**: A state's hue shall remain identifiable under the graph's lighting, so five states read as five colours rather than five gradations of one.
- [x] **UI-GRAPH3D-020**: Double-clicking a skill shall enter POV traversal: the camera shall fly to stand beside the skill with the orbit controls released and the lens opened, and the skill's parents and children shall appear as floating cards in the world.
- [x] **UI-GRAPH3D-021**: A neighbour card shall name its skill and carry its state and current progress; clicking an unlocked one shall move the camera to it, while clicking a locked or structural neighbour shall leave the camera in place and show a notice naming the current skill that must be completed first.
- [x] **UI-GRAPH3D-022**: The learner shall enter a skill's realm from its own card, and shall leave POV by one deliberate action that returns the camera to the overview; where the viewer has asked for less motion, both journeys shall be skipped rather than shortened.
- [x] **UI-GRAPH3D-023**: A skill's realm shall be walked the same way the tree is: the camera shall stand at the current lesson with the chain rising ahead, clicking an open lesson shall open its lesson card for practice without moving the camera, double-clicking it shall walk the camera to it only after that lesson is cleared, and right-drag shall look around; a closed lesson shall remain visible with progress but clicking or double-clicking it shall leave the camera in place and explain that the prerequisite is incomplete.
- [x] **UI-GRAPH3D-026**: Entering a realm shall turn the camera around rather than slide it: the walk-in shall swing the camera one hundred and eighty degrees about the skill while closing from the dive distance to the standing point, keeping the skill in frame for the whole turn; where the viewer has asked for less motion, the standing pose shall be handed over directly.
- [x] **UI-GRAPH3D-027**: A realm's cards shall carry a lesson's state and current progress — cleared with its best score, open, or locked — and the test card shall carry the test's state, and no state text shall float beneath the discs.
- [x] **UI-GRAPH3D-028**: Entering a realm shall fade the graph fully to black before the world swaps, then fade the realm in; reduced-motion viewers shall receive the destination without the decorative fade.
- [x] **UI-GRAPH3D-029**: The graph, its traversal controls, its POV cards, and its lesson realm shall use the site's warm light surfaces and shared sakura, plum, green, violet, amber, blue, and warm-neutral state palette consistently.
- [x] **UI-GRAPH3D-024**: In the overview, a single click on a skill shall open a modal naming it, describing it, and offering to enter its world; the two-click gesture that enters POV traversal shall be disambiguated from the one-click gesture by a short delay, so a double-click never summons the modal.
- [x] **UI-GRAPH3D-025**: A realm's lesson chain shall be walked neighbour to neighbour: standing at a lesson, the lesson the line connects it to — the one ahead and the one behind — shall carry a floating card naming it, showing what it asks for and its current progress, and clicking a card shall open that lesson's card for practice when it is open while double-clicking shall move the camera to it only when that target lesson is cleared; a closed card shall explain the prerequisite instead of moving the camera, and the test shall hang above the last lesson as its own card.
- [x] **UI-GRAPH3D-030**: While standing on a lesson node, the realm shall show a bottom action labelled `Start lesson` when it has not been cleared and `Redo lesson` when it has, and that action shall open the standing lesson's practice flow; an open future lesson shall instead be started from its own card, and only a cleared target lesson can become the standing node.
- [x] **UI-GRAPH3D-031**: The course graph viewport shall not render the course search control over the canvas, so projected skill and lesson hover information remains unobstructed in the top-left corner.
